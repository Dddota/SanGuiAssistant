...
class GuixinTab(QWidget):
    def __init__(self, runner: TaskRunner, parent=None):
        super().__init__(parent)
        self.setObjectName("guixinTab")
    ...
    def _init_ui(self) -> None:
        ...
        title = TitleLabel("一键归心")
        layout.addWidget(title)

        subtitle = SubtitleLabel("自动把所有城池的归心次数用完")
        ...
        self.start_btn = PrimaryPushButton("开始归心")
    ...
    def _on_start(self) -> None:
        ...
        self.progress.setValue(0)
        self.status_label.setText("状态：归心中...")
        self._append_log("=" * 40)
        self._append_log("开始执行归心任务")

        self.runner.start(task_name="guixin_start")
    ...
    def _on_task_finished(self, success: bool) -> None:
        ...
        if success:
            self.status_label.setText("状态：归心完成")
            self._append_log("归心任务完成")
            InfoBar.success(
                title="完成",
                content="归心任务执行完毕",
                orient=Qt.Orientation.Horizontal,
